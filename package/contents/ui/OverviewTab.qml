    ...
    contentItem: Flickable {
        ...
        ColumnLayout {
            ...
            PlasmaExtras.PlaceholderMessage {
                ...
                text: i18nc("@info:placeholder", "No API key configured")
                ...
                ColumnLayout {
                    ...
                    PlasmaExtras.DescriptiveLabel {
                        text: overview.api.creditsValid
                            ? i18nc("@label", "Available credit")
                            : i18nc("@label", "Remaining key limit")
                    }
                    ...
                    RowLayout {
                        ...
                        PlasmaComponents3.Button {
                            text: i18nc("@action:button", "Buy credit")
                    ...
                    PlasmaExtras.DescriptiveLabel {
                        ...
                        Layout.fillWidth: true
                        elide: Text.ElideRight
                        text: i18nc("@info %1 is the spent amount, %2 the purchased amount, %3 a percentage",
                                    "%1 of %2 spent (%3%)",
                    ...
                    PlasmaExtras.DescriptiveLabel {
                        ...
                        text: i18nc("@info", "The real account balance needs a management key (Settings → Account).")
                    }
                }
            }
            ...
            GridLayout {
                ...
                readonly property string scopeHint: overview.api.accountWide
                    ? i18nc("@info:whatsthis data scope", "whole account")
                    : i18nc("@info:whatsthis data scope", "this key only")

                StatTile {
                    caption: i18nc("@label spending period", "Today")
                    value: l10n.money(overview.api.usageTodayEff, overview.cfg.decimals, overview.sym)
                    hint: overview.api.activityLoaded && !overview.api.activityHasToday
                        ? i18nc("@info:whatsthis data scope", "this key only") : usageGrid.scopeHint
                }
                StatTile {
                    caption: i18nc("@label spending period", "This week")
                    value: l10n.money(overview.api.usageWeekEff, overview.cfg.decimals, overview.sym)
                    hint: usageGrid.scopeHint
                }
                StatTile {
                    caption: i18nc("@label spending period", "This month")
                    value: l10n.money(overview.api.usageMonthEff, overview.cfg.decimals, overview.sym)
                    hint: usageGrid.scopeHint
                }
                StatTile {
                    caption: i18nc("@label spending period", "Spent in total")
            ...
            PlasmaExtras.Heading {
                ...
                text: i18nc("@title:group", "API key")
                level: 5
                Layout.fillWidth: true
                Layout.topMargin: Kirigami.Units.smallSpacing
            }
            ...
            ColumnLayout {
                ...
                InfoRow {
                    ...
                    value: overview.api.keyLabel.length > 0
                        ? overview.api.keyLabel : i18nc("@item key without a name", "unnamed")
                }
                InfoRow {
                    label: i18nc("@label", "Spending limit")
                    value: overview.api.keyLimit === null
                        ? i18nc("@item no spending limit set", "unlimited")
                        : l10n.money(overview.api.keyLimit, 2, overview.sym)
                }
                InfoRow {
                    visible: overview.api.keyLimitRemaining !== null
                    label: i18nc("@label", "Left of that")
                    value: l10n.money(overview.api.keyLimitRemaining, 2, overview.sym)
                    emphasized: true
                }
                InfoRow {
                    visible: overview.api.keyLimitReset.length > 0
                    label: i18nc("@label", "Limit resets")
                    value: overview.api.keyLimitReset
                }
                InfoRow {
                    label: i18nc("@label account tier", "Tier")
                    value: overview.api.freeTier
                        ? i18nc("@item account tier", "free tier")
                        : i18nc("@item account tier", "paid")
                }
                InfoRow {
                    ...
                    label: i18nc("@label", "Rate limit")
                    value: overview.api.rateLimit
                        ? i18nc("@item %1 is a request count, %2 an interval such as 10s",
                                "%1 requests / %2",
                ...
                InfoRow {
                    ...
                    label: i18nc("@label bring-your-own-key usage", "BYOK total")
                        ...
                        value: {
                            if (!modelData.model) {
                                return overview.api.modelsLoaded
                                    ? i18nc("@item model id that is not in the catalogue", "unknown") : "…";
                            }
            ...
            PlasmaExtras.DescriptiveLabel {
                ...
                text: {
                    ...
                    if (overview.api.lastUpdate <= 0) {
                        return i18nc("@info:status", "No data fetched yet");
                    }
                    return i18nc("@info:status %1 is a relative time such as '5 minutes ago'",
